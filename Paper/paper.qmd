...
```{r}
# If you don't have these libraries downloaded use the install.packages.R script in scripts/supporting code
```
...
```{r}
# Generates ggplot ,strip.position = "left"
...
ggplot(data = river_long %>% 
  ...
  geom_line() + facet_wrap(~contaminant, scales = "free_y", ncol = 1,strip.position = "left" ) +
  theme_linedraw()+ labs(x = "Year", y = "Rolling Means", color = "Sample ID") + 
